...
function searchVOD(options) {
    ...
    return getVODMeta(options.vodId, options.clientId).then((meta) => {
        var diff = (options.start - meta.start) / 1000;
        ...
        if(options.end) {
            if(options.end < meta.start)
                options.length = Math.floor((options.end - options.start) / 1000);
            else
                options.length = Math.floor((options.end - meta.start) / 1000);
        }

        return searchChat(Math.floor(meta.start / 1000), Math.min(options.length, meta.length - diff), options.vodId, options.colorize, options.progress)
    });
}
