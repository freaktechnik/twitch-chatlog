...
function getVODMeta(vodID, clientId, testServer) {
    return fetch(getHost("https://api.twitch.tv", testServer) + "/kraken/videos/" + vodID, {
        ...
        if(resp.ok) {
            ...
            throw new Error("Could not load VOD details: " + resp.statusText);
        }
    }).then((json) => {
        return {
    ...
    return fetch(getHost("https://rechat.twitch.tv", testServer) + "/rechat-messages?start=" + start + "&video_id=v" + vodID)
        .then((resp) => {
            if(resp.ok) {
                ...
                throw new Error("Could not load fragment at " + start + ": " + resp.statusText);
            }
        }).then((json) => {
            return json.data;
        });
...
function formatMessage(msg, c) {
    ...
    if(c) {
        return [ chalk.gray(timestamp), chalk.bold(colorize(username, msg.attributes.color)), message ].join(" ");
    }
    else {
