...
async function searchVOD(options) {
    ...
    return searchChat(Math.floor(diff), Math.min(options.length, meta.length - diff), options.vodId, options.clientId, options.progress, options.testServer, meta.start);
}
exports.getChatlog = searchVOD;

function cheerTextColors(amount) {
    if(amount < 10000) {
        return '#292141';
    }
...
function cheerCharacter(amount) {
    if(amount < 100) {
        return '▴'; // triangle
    }
    else if(amount < 1000) {
...
function formatBadge(badgeInfo) {
    ...
    case "globalmod": return chalk.hex('#FFFFFF').bgHex('#016e2b')("🗡 "); //TODO not sure on the id and should be an axe...
    ...
    case "bits": return chalk
        .hex(cheerTextColors(badgeInfo.version))
        .bgHex(cheerColors(badgeInfo.version))(`${cheerCharacter(badgeInfo.version)} `);
