...
async function searchChat(start, length, vodID, clientId, colorized = false, progress = false, testServer) {
    let fragment,
        bar,
...
async function searchVOD(options) {
    ...
    return searchChat(Math.floor(diff), Math.min(options.length, meta.length - diff), options.vodId, options.clientId, options.colorize, options.progress, options.testServer);
}
exports.getChatlog = searchVOD;

function formatBadge(badgeInfo) {
    switch(badgeInfo._id) {
    case "premium": return chalk.white.bgBlue("👑 ");
    case "turbo": return chalk.white.bgMagenta("🔋 ");
    case "moderator": return chalk.white.bgGreen("⚔️ ");
    case "staff": return chalk.white.bgYellow("🛡 ");
    default: return "";
    }
}
...
function formatMessage(msg, c) {
    ...
    const timestamp = `[${new Date(msg.created_at).toTimeString()}]`,
        username = `${badges}<${msg.commenter.display_name || msg.commenter.name}>`,
        { body: message } = msg.message;
    if(c) {
        return [
            chalk.gray(timestamp),
            chalk.bold(colorize(username, msg.message.user_color)),
            message
        ].join(" ");
    }
