...
async function formatMessage(msg, c = false, loadImages = false) {
    let badges = "";
    ...
    if(c) {
        ...
        if(loadImages && msg.message.fragments && msg.message.fragments.length) {
 ...
 */
async function printResults(results, colorized, progress, loadImages) {
    let bar;
    if(progress) {
        bar = ora('Building messages...').start();
    }
    const formattedMessages = await Promise.all(results.map((msg) => formatMessage(msg, colorized, loadImages)));
    if(bar) {
