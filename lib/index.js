...
function getChatFragment(start, vodID, clientId, testServer) {
    ...
    return fetch(`${getHost("https://api.twitch.tv", testServer)}/kraken/videos/${vodID}/comments?${parameter}`, {
        headers: {
            "Client-ID": clientId,
            "Accept": "application/vnd.twitchtv.v5+json"
        }
...
async function searchVOD(options) {
    ...
    if(options.vodId[FIRST_CHAR] == VOD_ID_PREFIX) {
        options.vodId = options.vodId.slice(VOD_ID_PREFIX.length);
...
async function getCheerEmotes(channelId, clientId, testServer) {
    const response = await fetch(`${getHost("https://api.twitch.tv", testServer)}/kraken/bits/actions?channel_id=${channelId}`, {
        headers: {
            "Client-ID": clientId,
            "Accept": "application/vnd.twitchtv.v5+json"
        }
    ...
    console.warn("Could not fetch bits for channel", channelId, response.status);
    return [];
}
...
async function printResults(results, colorized, progress, loadImages, clientId, testServer) {
    ...
    const cheerEmotes = [],
        formattedMessages = await Promise.all(results.map((message) => formatMessage(message, colorized, loadImages, cheerEmotes, clientId, testServer)));
    if(bar) {
